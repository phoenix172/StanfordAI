...
namespace AI.Tests
{
    public class Tests
    {
        ...
        public void Test()
        {
            ...
            {
                var numbers =
                    Vector<double>.Build.DenseOfEnumerable(Enumerable.Range(0, 4)
                        .Select(_ => (double)rand.Next(0, 10)));
                return numbers;
            }).ToList();
            ...
            var target =
                Vector<double>.Build.DenseOfEnumerable(inputArray.Select(x => (double)(x.Sum() % 2 == 0 ? 2 : 1)));
